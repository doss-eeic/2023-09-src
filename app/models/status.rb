...
class Status < ApplicationRecord
  ...
  has_many :thumbsdowns, inverse_of: :status, dependent: :destroy
  ...
  class << self
    ...
    def thumbsups_map(status_ids, account_id)
      Thumbsup.select('status_id').where(status_id: status_ids).where(account_id: account_id).each_with_object({}) { |f, h| h[f.status_id] = true }
    end

    def thumbsdowns_map(status_ids, account_id)
      Thumbsdown.select('status_id').where(status_id: status_ids).where(account_id: account_id).each_with_object({}) { |f, h| h[f.status_id] = true }
    end
